...
function PriceRow({
  ...
  return (
    <div
      className={`flex items-center justify-between py-2.5 border-b border-gray-100 last:border-0 ${accent ? "font-semibold text-gray-900" : "text-gray-500"}`}
    >
      <span className="text-sm">{label}</span>
      <span
        className={`text-sm ${accent ? "text-[#8b5e2f] text-base font-bold" : ""}`}
      >
        {value}
      </span>
    </div>
        ...
        <img
          ...
          onError={(e) => {
            (e.target as HTMLImageElement).src =
              "https://placehold.co/300x300/f5f0eb/c9a96e?text=Jewellery";
          }}
        />
      </div>
      <div className="px-3 py-2.5">
        <p className="text-[13px] font-semibold text-gray-800 line-clamp-1 group-hover:text-amber-700 transition-colors">
          {product.name}
        </p>
      ...
      const newScale = Math.max(
        ...
        Math.min(
          ...
          pinchRef.current.initialScale *
            (newDistance / pinchRef.current.initialDistance),
        ),
      );
      setScale(newScale);
...
function ZoomImage({
  ...
  const handleMouseEnter = () => {
    if (!isTouchDevice) setZoom((z) => ({ ...z, active: true }));
  };
          ...
          {zoom.active && !isTouchDevice && size.w > 0 && (
            <div
              ...
              style={{
                ...
                boxShadow:
                  "0 0 0 1px rgba(0,0,0,0.2), 0 6px 24px rgba(0,0,0,0.25)",
              }}
            />
          )}
...
export default function ProductDetailPage() {
  ...
  const [toast, setToast] = useState<{
    message: string;
    type: "add" | "remove" | "cart";
  } | null>(null);
  const [addedToCart, setAddedToCart] = useState(false);
  useEffect(() => {
    if (!slug) return;

    const fromAssets = PRODUCTS.find((p) => p.slug === slug);
  ...
  const isWishlisted = wishlistItems.some(
    (i) => i.productId === product._id || product.id,
  );
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 xl:gap-14">
                ...
                {uniqueImages.map((img, idx) => (
                  ...
                  >
                    <img
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src =
                          "https://placehold.co/64x64/f5f0eb/c9a96e?text=+";
                      }}
                    />
                  </button>
                ))}
          ...
          <div className="flex flex-col gap-5">
              ...
              {[
                {
                  icon: (
                    <span
                      className="iconify text-amber-600"
                      data-icon="lucide:scale"
                      data-width="14"
                    />
                  ),
                  label: `${product.netWeight}.000 gm`,
                },
                {
                  icon: (
                    <span
                ...
                <span
                  key={spec.label}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white border border-gray-200 text-[13px] text-gray-700 font-medium shadow-sm"
                >
                  {spec.icon}
                  {spec.label}
                </span>
            ...
            <div className="rounded-2xl border border-[#ECE8E1] bg-white overflow-hidden">
              {/* Header */}
              <div className="px-4 py-3 border-b border-[#ECE8E1]">
                <h2 className="text-[13px] font-semibold uppercase text-[#A06A2C]">
                  About This Product
                </h2>
              </div>
              ...
              <div className="px-5 py-4">
                <p className="text-[13px] leading-7 text-gray-600">
                  {product.description ||
                    "Crafted with exceptional attention to detail, this elegant gold ring reflects timeless craftsmanship and refined aesthetics. Designed for lasting comfort and everyday elegance, it is a versatile piece that complements both traditional and contemporary styles."}
                </p>
              </div>
            </div>
            <div className="rounded-2xl border border-[#ECE8E1] bg-white overflow-hidden">
              {/* Header */}
              <div className="px-5 py-3 border-b border-[#ECE8E1]">
                <h3 className="text-[13px] font-semibold uppercase text-[#A06A2C]">
                  Specifications
                </h3>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2">
                ...
                <div>
                  <div className="flex items-center justify-between px-5 py-3 border-b border-[#F2F2F2]">
                    <span className="text-[13px] text-gray-500">Weight</span>
                    <span className="text-[13px] font-medium text-gray-900">
                      {product.netWeight} / gm
                    </span>
                  </div>

                  <div className="flex items-center justify-between px-5 py-3 border-b border-[#F2F2F2]">
                    <span className="text-[13px] text-gray-500">Purity</span>
                    <span className="text-[13px] font-medium text-gray-900">
                      {product.purity}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between px-5 py-3">
                    <span className="text-[13px] text-gray-500">Metal</span>
                    <span className="text-[13px] font-medium text-gray-900">
                      {product.metal}
                    </span>
                  </div>
                ...
                {/* Right */}
                <div className="md:border-l border-[#ECE8E1]">
                  <div className="flex items-center justify-between px-5 py-3 border-b border-[#F2F2F2]">
                  ...
                  <div className="flex items-center justify-between px-5 py-3">
                    <span className="text-[13px] text-gray-500">SKU</span>
                    <span className="text-[13px] font-medium text-gray-900">
                      {product.sku}
                    </span>
                  </div>
  ...
  title: "Handcrafted",
  sub: "Expertly handcrafted",
}
              ].map((badge) => (
                <div
                  key={badge.title}
                  className="flex flex-col items-center text-center p-3 rounded-2xl bg-white border border-gray-100 shadow-sm"
                >
