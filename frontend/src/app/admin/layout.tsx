...
const allNavItems = [
  ...
  },
  {
    ...
    icon: Home,
    module: "website" as const,
    roles: ["customer", "sales_staff", "inventory_manager"],
  },
  {
    href: "/admin/users",
    label: "Users & Roles",
...
function SidebarContent({
  ...
  return (
    <div className="flex flex-col h-full min-h-0 overflow-hidden">
      ...
      <div
        className={`flex-shrink-0 flex items-center gap-2 px-3 py-3 border-b border-[#C9A84C]/20 ${collapsed ? "justify-center" : "pr-12"}`}
      >
          ...
          <div>
            <Link href="/" className="flex items-center flex-shrink-0 min-w-0">
              <img
                src="/logo.jpg"
                alt="Ratan Jewellers"
                className="h-[95px] sm:h-[115px] md:h-[135px] w-auto object-contain max-w-[180px] sm:max-w-[210px] md:max-w-[240px]"
              />
            </Link>
            <div className="px-4 sm:px-6 md:px-8 mt-0">
  <p className="text-[11px] sm:text-xs md:text-sm font-extrabold uppercase tracking-[0.16em] md:tracking-[0.2em] text-[#C7BCA8] whitespace-nowrap text-left">
    ADMIN PANEL
  </p>
</div>
          </div>
      ...
      {!collapsed && currentUser.role === "super_admin" && (
        <div className="flex-shrink-0 px-3 py-2 border-b border-white/5">
          <div className="text-white/30 text-[9px] uppercase tracking-widest mb-1.5 px-1">
            View As Role
          </div>
          <div className="relative">
            ...
            >
              <span
                className={`text-xs font-semibold px-2 py-0.5 rounded-full ${roleColors[effectiveRole]}`}
              >
                {roleLabels[effectiveRole]}
              </span>
              <ChevronDown
                size={12}
                className={`text-white/40 transition-transform ${roleDropOpen ? "rotate-180" : ""}`}
              />
            </button>
            {roleDropOpen && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-[#1a0e00] border border-white/10 rounded-lg z-50 shadow-xl max-h-[35vh] overflow-y-auto">
                ...
                >
                  <span
                    className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${roleColors["super_admin"]}`}
                  >
                    Super Admin (default)
                  </span>
                </button>
                {(Object.keys(RBAC) as AdminRole[])
                  ...
                  .map((role) => (
                    ...
                    >
                      <span
                        className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${roleColors[role]}`}
                      >
                        {roleLabels[role]}
                      </span>
                    </button>
      ...
      {!collapsed && currentUser.role !== "super_admin" && (
        <div className="flex-shrink-0 px-3 py-2 border-b border-white/5">
          ...
          <div className="px-3 py-2 rounded-lg bg-white/5">
            <span
              className={`text-xs font-semibold px-2 py-0.5 rounded-full ${roleColors[currentUser.role]}`}
            >
          ...
          return (
            <div key={href} className="relative group">
              {accessible ? (
                ...
                <div
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm opacity-25 cursor-not-allowed ${collapsed ? "justify-center" : ""}`}
                >
                  <Icon size={17} className="flex-shrink-0" />
...
export default function AdminLayout({
  ...
  const unreadCJ = cjRequests.filter(
    (r) => !r.readByManager && r.status === "new",
  ).length;
  ...
  useEffect(() => {
    ...
    if (pathname === "/admin" || pathname === "/admin/") {
      router.replace(
        ["customer", "sales_staff", "inventory_manager"].includes(
          currentUser.role,
        )
          ? "/admin/my-dashboard"
          : "/admin/dashboard",
  ...
  if (!hydrated || !isLoggedIn || !currentUser)
    return (
  ...
  return (
    <div
      className="flex h-screen bg-[#F5F0E8] overflow-hidden"
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 pl-3 border-l border-gray-200">
              ...
              <div className="hidden sm:block">
                ...
                <div
                  className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${roleColors[effectiveRole]}`}
                >
                  {roleLabels[effectiveRole]}
                </div>
              </div>
