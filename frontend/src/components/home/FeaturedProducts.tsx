...
export default function FeaturedProducts({ title, filter }: Props) {
  const { data, isLoading } = useQuery({ queryKey: ['products', filter], queryFn: () => api.get<ProductsResponse>(`/products?${filter==='trending'?'trending=true':filter==='featured'?'featured=true':'sortBy=createdAt'}&limit=8`), retry: false })
    ...
    <section className="py-16 sm:py-20 bg-cream">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">{products.map((product, i) => <motion.div key={product.id} initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ delay:i*0.06, duration:0.5 }}><ProductCard product={product} /></motion.div>)}</div>
        )}
