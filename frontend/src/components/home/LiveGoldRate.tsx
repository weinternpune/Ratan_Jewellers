    ...
    <div className="bg-obsidian border-y border-gold/20 py-3 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="hidden md:flex items-center gap-4 overflow-x-auto">
          ...
          <div className="ml-auto flex-shrink-0 flex items-center gap-2"><span className="text-[10px] text-gold/30 font-mono-code">Updated {lastUpdated.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</span><button onClick={refreshRate} disabled={isUpdating} className="text-gold/40 hover:text-gold transition-colors"><RefreshCw size={12} className={isUpdating ? 'animate-spin' : ''} /></button></div>
        </div>

        {/* Mobile Version - With Auto-Scroll Animation */}
        <div className="md:hidden overflow-hidden relative">
          <div className="gold-rates-scroll flex items-center gap-4">
            {/* First Set */}
            <div className="flex items-center gap-4 flex-shrink-0">
              <div className="flex items-center gap-2"><div className="w-2 h-2 bg-green-400 rounded-full animate-pulse" /><span className="font-mono-code text-xs text-gold/70 uppercase tracking-wider whitespace-nowrap">Live Gold Rates</span></div>
              <div className="w-px h-4 bg-gold/20" />
              <div className="flex items-center gap-6">{purities.map(p => <div key={p.label} className="flex items-center gap-2 whitespace-nowrap"><span className="font-mono-code text-xs text-gold/50">{p.label}</span><span className="font-mono-code text-sm text-gold font-medium">₹{Math.round(goldRate * p.multiplier).toLocaleString('en-IN')}<span className="text-gold/50 text-[10px]">/g</span></span></div>)}</div>
              <div className="w-px h-4 bg-gold/20" />
              <div className={`flex items-center gap-1 ${trend === 'up' ? 'text-green-400' : 'text-red-400'}`}>{trend === 'up' ? <TrendingUp size={13} /> : <TrendingDown size={13} />}<span className="font-mono-code text-xs">{change >= 0 ? '+' : ''}{change.toFixed(0)} ({changePercent}%)</span></div>
            </div>
            ...
            <div className="flex items-center gap-4 flex-shrink-0">
              ...
              <div className="flex items-center gap-2"><div className="w-2 h-2 bg-green-400 rounded-full animate-pulse" /><span className="font-mono-code text-xs text-gold/70 uppercase tracking-wider whitespace-nowrap">Live Gold Rates</span></div>
              <div className="w-px h-4 bg-gold/20" />
              <div className="flex items-center gap-6">{purities.map((p, idx) => <div key={`dup-${idx}`} className="flex items-center gap-2 whitespace-nowrap"><span className="font-mono-code text-xs text-gold/50">{p.label}</span><span className="font-mono-code text-sm text-gold font-medium">₹{Math.round(goldRate * p.multiplier).toLocaleString('en-IN')}<span className="text-gold/50 text-[10px]">/g</span></span></div>)}</div>
              <div className="w-px h-4 bg-gold/20" />
              <div className={`flex items-center gap-1 ${trend === 'up' ? 'text-green-400' : 'text-red-400'}`}>{trend === 'up' ? <TrendingUp size={13} /> : <TrendingDown size={13} />}<span className="font-mono-code text-xs">{change >= 0 ? '+' : ''}{change.toFixed(0)} ({changePercent}%)</span></div>
            </div>
            ...
            <div className="flex items-center gap-4 flex-shrink-0">
              ...
              <div className="flex items-center gap-2"><div className="w-2 h-2 bg-green-400 rounded-full animate-pulse" /><span className="font-mono-code text-xs text-gold/70 uppercase tracking-wider whitespace-nowrap">Live Gold Rates</span></div>
              <div className="w-px h-4 bg-gold/20" />
              <div className="flex items-center gap-6">{purities.map((p, idx) => <div key={`dup2-${idx}`} className="flex items-center gap-2 whitespace-nowrap"><span className="font-mono-code text-xs text-gold/50">{p.label}</span><span className="font-mono-code text-sm text-gold font-medium">₹{Math.round(goldRate * p.multiplier).toLocaleString('en-IN')}<span className="text-gold/50 text-[10px]">/g</span></span></div>)}</div>
              <div className="w-px h-4 bg-gold/20" />
              <div className={`flex items-center gap-1 ${trend === 'up' ? 'text-green-400' : 'text-red-400'}`}>{trend === 'up' ? <TrendingUp size={13} /> : <TrendingDown size={13} />}<span className="font-mono-code text-xs">{change >= 0 ? '+' : ''}{change.toFixed(0)} ({changePercent}%)</span></div>
            </div>
          </div>
          
          {/* Refresh Button - Fixed on mobile */}
          <div className="absolute right-0 top-1/2 -translate-y-1/2 bg-obsidian pl-4 pr-2">
            <button onClick={refreshRate} disabled={isUpdating} className="text-gold/40 hover:text-gold transition-colors">
