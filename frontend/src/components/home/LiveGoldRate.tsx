        ...
        <div ref={scrollContainerRef} className="flex items-center gap-4 overflow-x-auto scrollbar-hide">
          ...
          <div className="ml-auto flex-shrink-0 flex items-center gap-2">
            <span className="text-[10px] text-gold/30 font-mono-code hidden sm:block">
              Updated {lastUpdated ? lastUpdated.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) : '--:--'}
            </span>
            <button onClick={refreshRate} disabled={isUpdating} className="text-gold/40 hover:text-gold transition-colors"><RefreshCw size={12} className={isUpdating ? 'animate-spin' : ''} /></button>
          </div>
        </div>
      </div>
    </div>
