...
//       if (!user) throw new AppError('No account found with this number. Please register first.', 404);
//     }
//     if (purpose === 'register') {
//       const field = type === 'phone' ? { phone: identifier } : { email: identifier };
//       const user = await User.findOne(field);
//       if (user) throw new AppError('Account already exists. Please login instead.', 409);
//     }
...
//     const field = isPhone ? { phone: identifier } : { email: identifier };
//     const user = await User.findOne(field);
//     if (!user) throw new AppError('User not found', 404);
...
// ✅ UPDATED: Include role in token payload and return expiry dates
const generateTokens = (userId: string, role: string = 'customer') => {
  ...
  if (!JWT_SECRET || !JWT_REFRESH_SECRET) {
    throw new AppError(
      'JWT secrets are not configured. Check your .env file.',
      500
    );
  }
...
export const verifyOTPHandler = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (purpose === 'register') {
      ...
      const userData: any = { name, isVerified: true, role: 'customer' };
...
export const googleCallback = async (req: Request, res: Response) => {
  try {
    ...
    // ✅ Include expiry dates in redirect
    res.redirect(
      `${process.env.FRONTEND_URL}/auth/callback?accessToken=${tokens.accessToken}&refreshToken=${tokens.refreshToken}&accessExpiresAt=${tokens.accessExpiresAt.toISOString()}&refreshExpiresAt=${tokens.refreshExpiresAt.toISOString()}`
    );
  } catch (err) {
...
export const refreshToken = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const JWT_REFRESH_SECRET = process.env.JWT_REFRESH_SECRET;
    if (!JWT_REFRESH_SECRET) throw new AppError('JWT secrets are not configured', 500);
    ...
    // ✅ Pass role when generating new tokens
