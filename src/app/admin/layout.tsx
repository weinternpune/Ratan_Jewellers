...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Validate token with backend on every page load
  useEffect(() => {
    if (pathname === '/admin/login') { setHydrated(true); return }
  ...
  return (
    <div className="flex h-screen bg-[#F5F0E8] overflow-hidden" style={{fontFamily:"'Inter', sans-serif"}}>
      {/* Desktop Sidebar */}
      <aside className={`hidden lg:flex flex-col flex-shrink-0 bg-[#0D0700] border-r border-[#C9A84C]/10 transition-all duration-300 relative ${collapsed?'w-16':'w-60'}`}>
        <SidebarContent/>
